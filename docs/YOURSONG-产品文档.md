...
| 方案 | 代价 | 结果 |
| --- | --- | --- |
| **A（推荐）** 你先装 Node，P0 直接在 **Nuxt** 里做 | 你花 5 分钟 `brew install node` | 一步到位，P2 不用搬家 |
| B 继续在现在这套无构建里做，P2 再搬 | 现在零成本 | 之后多花约半天把组件搬成 Nuxt 的写法 |

差别不大，但 A 更干净。**你装完 Node 跟我说一声，我就按 A 走**；不想装就说一声，我按 B 开始。
